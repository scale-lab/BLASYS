...
def synth_design(input_file, output_file, lib_file):
 ...
 #   f.write('espresso;map')
    ...
    f=open('yosys.script', 'w')
    yosys_command = 'read_verilog ' + input_file + '; ' \
            + 'synth -flatten; opt; opt_clean -purge; techmap; abc -liberty '+lib_file + ' -script abc.script; ' \
...
#    synth_gen(sys.argv[1]+'_approx_k='+str(k)+'.v', sys.argv[1]+'_approx_k='+str(k)+'_syn', sys.argv[2])
